...
{
    image_sub_ptr_->ParseData(image_color_data_buff_, image_depth_data_buff_);   //Parse是解析的意思. 
...
{
    image_sub_ptr_->ParseData(image_color_data_buff_, image_depth_data_buff_);   //这两个变量是双端队列,使用方法与vector相似
    if (image_color_data_buff_.size() == 0 || image_depth_data_buff_.size() == 0)
        return false;
    return true;
}
