...
//{
//    image_sub_ptr_->ParseData(image_color_data_buff_, image_depth_data_buff_);   //Parse是解析的意思. 这里将image insert进了两个队列中.
...
//    // (1) image_time是 第一个frame的时间.  (2)如果return的是false,说明 IMU起始时间晚于图像时间时，会删除图像数据.
...
//{
//    double image_time;
//    if (rosBagFlag){  //TODO: 这不是代表着 不是实时运行的??
...
//    ros::param::param<std::string>("~depth_topic", depth_topic, "/camera/aligned_depth_to_color/image_raw");
    ros::param::param<std::string>("~rgb_topic", rgb_topic, "/rgb/image_raw");
    ros::param::param<std::string>("~depth_topic", depth_topic, "/depth_to_rgb/image_raw");
...
//    const std::string YamlFile = WORK_SPACE_PATH + "/ros_test/config/" + yamlfile;
    ...
    ROS_INFO("SUCCESS TO INIT SYSTEM!");
}
...
bool RGBDMessageFlow::ValidData()   //对队列中的 队首 image,进行处理.
{
    double image_time;
    if (rosBagFlag)
    {
