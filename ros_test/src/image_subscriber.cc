...
IMGSubscriber::IMGSubscriber(ros::NodeHandle &nh, std::string color_topic_name, std::string depth_topic_name, std::string bbox_topic_name, size_t buff_size)
    : it_(nh)
{   
    ROS_INFO("[DEBUG] IMGSubscriber Waiting Topic: %s, %s, %s \n",color_topic_name, depth_topic_name, bbox_topic_name);
...
// 回调函数
// 输入：RBG彩色图像和深度图像
void IMGSubscriber::image_callback(const sensor_msgs::ImageConstPtr &msgImg, const sensor_msgs::ImageConstPtr &msgDepth, const darknet_ros_msgs::BoundingBoxesConstPtr &msgBbox)
{
    ROS_INFO("IMGSubscriber Get Frame. ");
...
void IMGSubscriber::ParseData(std::deque<sensor_msgs::Image> &image_color_data_buff, std::deque<sensor_msgs::Image> &image_depth_data_buff,
                                 std::deque<darknet_ros_msgs::BoundingBoxes> &image_bbox_data_buff)
{
    buff_mutex_.lock();

    // pipe all available measurements to output buffer:
    if (new_color_data_.size() > 0)
    {   //TODO: 这是不是说明了,这个系统是 非实时的.  因为图像并不是实时处理的,而是会被缓存起来??  
        ...
        image_bbox_data_buff.insert(image_bbox_data_buff.end(), new_bbox_data_.begin(), new_bbox_data_.end());
        new_bbox_data_.clear();
