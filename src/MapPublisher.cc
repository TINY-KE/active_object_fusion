...
{
    {
        PublishCurrentCamera(*CurrentCameraPose);
    }
    {
...
void MapPublisher::PublishObject(const vector<Object_Map*> &vObjs ){
    ...
    for(size_t i=0; i< vObjs.size(); i++)
    {
        ...
        if((vObjs[i]->mvpMapObjectMappoints.size() < 10) || (vObjs[i]->bBadErase == true))
        {
            continue;
        }
        ...
        vector<MapPoint* > vpMPs = vObjs[i]->mvpMapObjectMappoints;
